...
| `[[error_translation]]` | Опциональный массив таблиц для замены тел ошибочных ответов от upstream. Каждая таблица: `status` (HTTP-статус), `ingress` (опциональная подстрока для поиска в теле ответа), `egress` (текст, на который заменяется тело). Правила проверяются по порядку, срабатывает первое совпадение. Если секция отсутствует или пуста — тела ошибок пробрасываются без изменений |

...
- `interactions_session_store` — optional path to session persistence TOML file
...
Provider sections can specify `endpoint_interactions` — a Gemini Interactions API endpoint. This is valid alongside or instead of `endpoint_openai` / `endpoint_anthropic`.
...
When routing to `endpoint_interactions`, the `api_key` config value is sent as `x-goog-api-key` header. Only three headers are sent:
...
No client request headers are forwarded.
...
- GIVEN client sent `Authorization: Bearer client-token`
- WHEN routing to interactions endpoint
- THEN no client headers appear in the upstream request
...
- GIVEN section has `control_clean_all` and `control_extend_lifetime`
- WHEN config is loaded
...
- WHEN config is loaded
- THEN in-band control is disabled

## Requirement: Interactions Egress Limit (proxy_limit)

Provider sections can specify `proxy_limit` — maximum byte size of the serialized `Content[]` array before splitting into multiple interactions.
...
- WHEN config is loaded
- THEN no size-based splitting is performed

## Requirement: Session Persistence Config

Session state is persisted to a TOML file. Override via global config key:

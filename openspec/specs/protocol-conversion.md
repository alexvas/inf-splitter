...
When a streaming session is marked pending with `interaction_id=""` and the client disconnects before `interaction.created` is parsed, the stream error path must clear the pending flag so startup recovery can remove the orphaned session.
...
`extract_anthropic_content` must handle `tool_result` content blocks in addition to text blocks. When a message's `content` array contains a block with `"type": "tool_result"`, the `content` field of that block (string or array of text blocks) is extracted as text.
...
`synthesize_openai_chunks` must emit `tool_calls` delta chunks when the translated response contains `tool_calls` in the choice message.
...
Interactions success responses must forward upstream rate-limit and trace headers to the client, matching the whitelist patterns used by passthrough paths.
...
### Scenario: Trace headers forwarded

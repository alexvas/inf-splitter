...
`forward_request_headers_map(api_key, headers, protocol)` in `auth.rs` maps client request headers to upstream request headers. `x-request-id` is **never** forwarded as-is — it is generated by the upstream. The `protocol` parameter determines the target correlation header.

**Source resolution** (same for all protocols):

Priority chain: `out.x-claude-code-session-id` (passthrough) > `headers.X-Client-Request-Id` > `headers.x-request-id`

**Target header** (protocol-specific):

| Protocol | Target header |
|----------|--------------|
...
- THEN `x-claude-code-session-id: client-1` is set in upstream headers
...
- THEN `X-Client-Request-Id: sess-1` is set in upstream headers
...
- WHEN forwarding to Anthropic upstream
- THEN `x-claude-code-session-id: sess-1` is forwarded as-is (passthrough)

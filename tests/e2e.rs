...
"#
    ...
    let typed: MessageResponse =
        serde_json::from_str(&body).expect("deserialize MessageResponse");
...
"#
    ...
    let seen_a = cap_a.lock().expect("lock cap_a").clone().expect("upstream a used");
    assert_eq!(seen_a["model"], "model-a");
    let seen_b = cap_b.lock().expect("lock cap_b").clone().expect("upstream b used");
